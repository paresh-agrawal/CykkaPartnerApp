...
public class SignUpActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LinearLayout tvSignIn = findViewById(R.id.ll_sign_in_text);
    ...
    private void updateUI(int uiState, FirebaseUser user, PhoneAuthCredential cred) {
        ...
        if(user!=null){
            ...
            Map<String, Object> detailsBool = new HashMap<>();
            detailsBool.put("DocumentsUploadedBool", false);
            detailsBool.put("ManualVerified", false);
            detailsBool.put("TotalRating", "0");
            detailsBool.put("TotalReviews", "0");
            detailsBool.put("Total5StarCount", "0");
            detailsBool.put("Total4StarCount", "0");
            detailsBool.put("Total3StarCount", "0");
            detailsBool.put("Total2StarCount", "0");
            detailsBool.put("Total1StarCount", "0");
            detailsBool.put("TotalClients", "0");
            detailsBool.put("TotalBlogs", "0");

            Map<String, Object> phoneNumberMap = new HashMap<>();
            phoneNumberMap.put(mPhoneNumber, "Advisor");
            ...
            DocumentReference docRef = db.collection("AdvisorsDatabase").document(user.getUid());
            if (isNetworkAvailable(getApplicationContext())) {
                docRef.get().addOnCompleteListener(task -> {
                    DocumentSnapshot document = task.getResult();
                    db.collection("AdvisorsPhoneNumbers").document("Users").update(phoneNumberMap);
                    if (Objects.requireNonNull(document).exists()) {
                        progressDialog.dismiss();
                        Log.d(TAG, "DocumentSnapshot successfully written!");
                        Intent intent = new Intent(SignUpActivity.this, IdentityVerificationActivity.class);
                        intent.putExtra("PARENT_ACTIVITY_TAG", SignUpActivity.TAG);
                        startActivity(intent);
                        finish();
                    } else {
                        docRef.set(detailsBool)
            ...
            /*startActivity(new Intent(SignUpActivity.this, ProfileDetailsActivity.class));
            finish();*/
                    ...
                    if (task.isSuccessful()) {
                        ...
                        if (Objects.requireNonNull(document).exists()) {
                            if (document.get(mPhoneNumber) != null) {
                                ...
                                progressDialog.dismiss();
                                mPhoneNumberField.setError("User with this phone number already exist");
                            } else {
                                progressDialog.dismiss();
